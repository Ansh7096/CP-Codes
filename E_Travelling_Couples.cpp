...
signed main()
{
    ...
    // male female both to reach
    vector<int> dis3(N + 1, intmax);
    priority_queue<pair<int, int>, vector<pair<int, int>>, greater<pair<int, int>>> pq;
    for (int i = 1; i <= N; i++)
    {
        if (dis1[i] != intmax && dis2[i] != intmax)
        {
    ...
    while (!pq.empty())
    {
        ...
        {
            if (dis3[it] > dis3[x.second] + C3)
            {
                dis3[it] = dis3[x.second] + C3;
                pq.push({dis3[it], it});
            }
        }
    }
