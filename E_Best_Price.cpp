...
void sol()
{
    int n,k;
        ...
        for(auto x : a) p.push_back({x, 1});
        for(auto x : b) p.push_back({x, -1});
        sort(p.begin(), p.end());
        //unique
        set<pair<int, int>>s;
        ...
        for(auto i : s) 
        {
            int ss = lower_bound(a.begin(), a.end(), i.first) - a.begin();
            int bb = lower_bound(b.begin(), b.end(), i.first) - b.begin();
            if(ss - bb <= k)
                ans = max(ans, i.first *(n-bb));
        }
        cout << ans << endl;
}
