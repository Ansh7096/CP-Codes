...
void sol()
{
   ...
   for(int i=0;i<n;i++)
   {
    for(int j=0;j<m;j++)
    {
        vv[v[i][j]]=max(vv[v[i][j]],1LL);
        for(int k=0;k<4;k++)
        {
        int nx=i+dx[k];
        int ny=j+dy[k];
     
        if(nx>=0&&nx<n&&ny>=0&&ny<m&&v[i][j]==v[nx][ny])
        {
         vv[v[i][j]]=2;
        }
        }
   ...
   if(*max_element(vv.begin(),vv.end())==1)
   cout<<accumulate(vv.begin(),vv.end(),0LL)-1<<endl;
   else
   cout<<accumulate(vv.begin(),vv.end(),0LL)-2<<endl;
}
